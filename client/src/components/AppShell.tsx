...
export type AppPage = "home" | "bank" | "practice" | "history" | "profile" | "settings" | "detail";
  ...
  return (
    <div className="app-shell">
      <header className="topbar">
        <button className="brand-mark" onClick={() => onNavigate("home")} aria-label="Vocabulary Master home">
          <img src={logoAsset} alt="Vocabulary Master" />
        </button>
        <div className="topbar-title"><span>VOCABULARY MASTER</span><strong>Study with clarity.</strong></div>
        <button className="profile-shortcut" onClick={onProfile} aria-label="Open profile"><UserRound size={18} /></button>
      </header>
      ...
      <nav className="bottom-nav" aria-label="Primary navigation">
        {navigation.map((item) => {
          ...
          return <button key={item.key} onClick={() => onNavigate(item.key)} className={`nav-item ${isActive ? "active" : ""} ${item.locked ? "locked" : ""}`} aria-current={isActive ? "page" : undefined}>
            <Icon size={19} strokeWidth={isActive ? 2.3 : 1.8} />
            <span>{item.label}</span>
            {item.locked && <i aria-hidden="true" />}
          </button>;
        })}
      </nav>
    </div>
