...
function ResultSummary({ record }: { record: ExamHistoryRecord }) {
  return <section className="exam-result-card"><p className="kicker">{record.status === "expired" ? "TIME'S UP" : "TEST COMPLETE"}</p><strong>{record.finalScore} <small>/ {record.totalQuestions}</small></strong><span>Final score</span><div><p><b>{record.accuracy}%</b><small>Accuracy</small></p><p><b>{record.correct}</b><small>Correct</small></p><p><b>{record.wrong}</b><small>Wrong</small></p><p><b>{record.skipped}</b><small>Skipped</small></p></div></section>;
}
...
function ExamQuestionCard({ question, index, answer, flagged, onAnswer, onFlag }: { question: ExamQuestion; index: number; answer?: string; flagged: boolean; onAnswer: (questionId: string, option: string) => void; onFlag: (questionId: string) => void }) {
  ...
  return <article className={`mock-question-card ${answer ? "answered" : ""}`} id={`mock-question-${index}`}>
    ...
    <div className="mock-options">
      {question.options.map((option, optionIndex) => <button key={`${question.id}-${option}`} type="button" className={answer === option ? "selected" : ""} onClick={() => onAnswer(question.id, option)} aria-pressed={answer === option}><span>{String.fromCharCode(65 + optionIndex)}</span><b>{option}</b></button>)}
    </div>
